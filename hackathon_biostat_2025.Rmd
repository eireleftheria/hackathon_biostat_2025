---
title: "Группа 4: Анна Вронская, Екатерина Данилина, Ирина Козлова, Сергей Царегородцев"
...
<h1 style="text-align: center;">Нежелательные явления при лечении болезни Крона</h1>

<h2 style="text-align: center;">1. Таблицы с базовыми описательными статистиками</h2>
...
cat_table <- CrohnD %>% 
  ...
  mutate(
    Всего = as.character(sum(as.numeric(`Количество`), na.rm = TRUE))
  ) %>%
  ungroup() %>%
  select(Препарат, Переменная, Всего, Уровень, `Количество`, `%`)

ft_cat <- flextable(cat_table) %>%
  merge_v(j = "Препарат") %>%
  merge_v(j = c("Переменная")) %>%  
  fontsize(size = 10) %>%
  bold(part = "header") %>%
  align(j = 1:2, align = "left", part = "all") %>%  
...
```{r BMI_AE,  fig.width=16, fig.height=9, echo=FALSE}
# p1 <- CrohnD %>% 
#   select(nrAdvE, BMI) %>% 
...
```{r height_AE,  fig.width=16, fig.height=9, echo=FALSE}
# p2 <- CrohnD %>% 
...
```{r country_AE,  fig.width=16, fig.height=9, echo=FALSE}
# p3 <- CrohnD %>% 
...
```{r hist_AE, fig.width=16, fig.height=6, echo=FALSE}
p5 <- CrohnD %>% 
  select(nrAdvE) %>% 
  ggplot() +
...
```{r hist_height, fig.width=16, fig.height=6, echo=FALSE}
p6 <- CrohnD %>% 
  select(height) %>% 
  ggplot() +
...
```{r hist_country, fig.width=6, fig.height=12, echo=FALSE}
p7 <- CrohnD %>% 
  select(country) %>% 
  ggplot() +
...
```{r hist_sex, fig.width=6, fig.height=12, echo=FALSE}
p8 <- CrohnD %>% 
  select(sex) %>% 
  ggplot() +
...
```{r hist_age, fig.width=16, fig.height=6, echo=FALSE}
p9 <- CrohnD %>% 
  select(age) %>% 
  ggplot() +
...
```{r hist_weight, fig.width=16, fig.height=6, echo=FALSE}
p10 <- CrohnD %>% 
  select(weight) %>% 
  ggplot() +
...
```{r BMI, fig.width=16, fig.height=6, echo=FALSE}
p12 <- CrohnD %>% 
  select(BMI) %>% 
  ggplot() +
  geom_density(aes(x = BMI), fill = "#D7E8BA") +
  theme_custom+
  #scale_x_continuous(breaks = seq(0, 120, by = 10))+
  labs(y = "Доля пациентов", x = "ИМТ, кг/м^2", title = "Распределение по ИМТ")

p12
```


## 3. Средства для визуальной оценки взаимосвязи между числом НЯ и собранными признаками пациентов.


```{r, fig.height=13, fig.width=13, error=FALSE, warning=FALSE, message=FALSE, echo=FALSE}
...
## 4. Методы однофакторного анализа для количественной оценки взаимосвязи между числом НЯ и собранными признаками пациентов (помните, что взаимосвязь - это не только p-значения, но и доверительные интервалы!).
...
cat("### Разница средних placebo и d1 \n")
...
# Выводим результаты
cat("### t-критерий для независимых выборок: НЯ между placebo и d2\n")
print(t_test_placebo_d2)

# Находим разницу средних для двух групп (placebo и d2)
cat("### Разница средних placebo и d2 \n")
...
# Выводим результаты
cat("### t-критерий для независимых выборок: НЯ между d1 и d2\n")
print(t_test_d1_d2)

# Находим разницу средних для двух групп (d1 и d2)
cat("### Разница средних d1 и d2 \n")
md_d1_d2 =  mean_d1 - mean_d2
md_d1_d2

# Корреляция между НЯ и BMI
cat("### Корреляция между НЯ и BMI\n")
cor(CrohnD$nrAdvE, CrohnD$BMI, method = "spearman")


# Корреляция между НЯ и возрастом
cat("### Корреляция между НЯ и возрастом\n")
cor(CrohnD$nrAdvE, CrohnD$age, method = "spearman")

# Регрессионная GLM модель из статьи
